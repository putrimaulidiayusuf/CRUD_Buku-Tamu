...
function b($sql){
  return mysqli_fetch_assoc(a($sql));
}

function tambahMahasiswa($a){
  $sql = "INSERT INTO kehadiran_siswa (nama, jurusan, nis)
          VALUES('$a[nama]', '$a[jurusan]', $a[nis])";
  return a($sql);
}

function getMahasiswa(){
  return a("SELECT * FROM kehadiran_siswa ORDER BY id DESC");
}

function getMahasiswaById($id){
  return b("SELECT * FROM kehadiran_siswa WHERE id=$id");
}

function updateMahasiswa($a){
  $sql = "UPDATE kehadiran_siswa
          SET nama='$a[nama]', jurusan='$a[jurusan]', nis=$a[nis]
          WHERE id=$a[id]";
  return a($sql);
}

function hapusMahasiswa($id){
  return a("DELETE FROM kehadiran_siswa WHERE id=$id");
}
  ...
  foreach(getMahasiswa() as $c): ?>
    <tr>
      ...
      <td>
        ...
        <a href="?hapus=<?= $c['id'] ?>" onclick="return confirm('Hapus?')">Hapus</a>
      </td>
    </tr>
  <?php endforeach ?>
</table>
